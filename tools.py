...
def _godot_project_cfg(project_name: str) -> str:
    """Return a valid Godot 4 project.godot config."""
...
def _godot_scene(root_type: str, script_name: str, extra_nodes: str = "") -> str:
    """Return a valid Godot 4 .tscn that roots to `root_type` with `script_name` attached."""
    return f"""[gd_scene load_steps=2 format=3]

[ext_resource type="Script" path="res://{script_name}" id="1_main"]
...
def check_godot_script(project_name: str, script_file: str = "") -> str:
    ...
    try:
        ...
        if not errors and not warnings:
            summary.append("No errors found — scripts look valid!")

        return "\n\n".join(summary) + f"\n\nFull output:\n{out}"
    except subprocess.TimeoutExpired:
        return "Godot check timed out."
    except FileNotFoundError:
        return "Error: Godot not found at PATH."
    except Exception as e:
...
def run_godot(project_name: str, editor: bool = False) -> str:
    ...
    try:
        if editor:
            ...
            boot_output = (proc.stdout + proc.stderr).strip()
            errors = [l for l in boot_output.splitlines()
                      if any(k in l.lower() for k in ("error", "failed", "invalid", "crashed"))]
...
def dispatch(tool_name: str, tool_input: dict) -> str:
    if tool_name == "write_file":
        ...
        return create_godot_project(
            ...
            tool_input["main_scene_script"],
            tool_input.get("root_node_type", "Node3D"),
            extra,
        )
